...
const DATA_FILE = "./interview-data.json";
...
async function readData() {
  try {
    ...
    return { accounts: [], users: [], interviews: [], questions: [] };
  }
}
...
app.post("/register", async (req, res) => {
  try {
    ...
    const exists =
      ...
      data.accounts.some((account) => account.username.toLowerCase() === normalizedUsername);
...
app.post("/save-user", async (req, res) => {
  try {
    ...
    res.json({ message: "User data saved successfully", id: user.id });
  } catch (error) {
...
app.get("/questions", async (req, res) => {
  try {
    const data = await readData();
    res.json(data.questions.slice().reverse());
  } catch (error) {
...
app.post("/questions", async (req, res) => {
  try {
    ...
    res.json({ message: "Question saved successfully", question: newQuestion });
  } catch (error) {
...
app.get("/question", async (req, res) => {
  try {
    const data = await readData();
    const user = data.users.at(-1);
    const matchingQuestions = user
      ? data.questions.filter(
          (item) =>
            item.domain.toLowerCase() === user.domain.toLowerCase() &&
            item.level.toLowerCase() === user.level.toLowerCase()
        )
      : data.questions;

    const customPool = matchingQuestions.length ? matchingQuestions : data.questions;

    if (customPool.length) {
      const savedQuestion = customPool[Math.floor(Math.random() * customPool.length)];
      return res.json({ question: savedQuestion.question, source: "saved" });
    }
...
app.get("/interviews", async (req, res) => {
  try {
    const data = await readData();
    res.json(data.interviews.slice(-5).reverse());
  } catch (error) {
...
app.get("/stats", async (req, res) => {
  try {
    const data = await readData();
    const scored = data.interviews.filter((item) => Number.isFinite(item.score));
    const total = scored.length;
    const average = total
      ? scored.reduce((sum, item) => sum + item.score, 0) / total
      : 0;
